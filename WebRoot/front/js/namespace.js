	...
	ESS = (function(){
		...
		return {
			...
			extend:function(name,parent,child){
				if(arguments.length != arguments.callee.length)	
					throw new Error("the number of actual arguments is not equal the expected.");
				if(this.isExist(name))	//�ж����ֿռ��Ƿ��Ѿ�����
					throw new Error("the namespace of ["+name+"] is already exist.");
				var ps = child.prototype, p;	//�������ඨ��ķ���
				child.prototype = Object.create(parent.prototype); //���������ԭ�Ͷ���
				for(p in ps){					//��ԭ���෽��
...
//				var searchExpr = /^([#\.]?[\w\-]+)(((\s*\:\s*|\s*\+\s*|\s*>\s*|\s+)([#\.]?[\w\-]+))+)?$/;
				...
				function Query(selector){
					return new proto.init(selector);  //�ı�������
				}
				//DOMElement
				 ...
				 * @param handler [string] DOMTokenList ���󷽷���(add �� remove)
				 */
...
ESS.register("ESS.ajax.request",function(o){
	...
	var method = o.method.toUpperCase(); //��Щ�������֧��Сд
...
/**
 * ��ͬ�����֮��EventTarget�еķ�����ͬ��ESS.event.Util����ʵ���������EventTarget������ͳһ
 */
ESS.register("ESS.event",function(){
	...
	function unifyEvent(e){
		/**
		 * �и��ر���Ҫע��ĵط�ʱ��IE���������mouserup��event.button = 2֮������ľ�Ϊ0
		 */
